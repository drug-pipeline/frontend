...
NGL.createParameterInput = function (p, v) {
  ...
  if (p.type === "number") {
    ...
    input = new UI.Integer(parseInt(value)).setRange(p.min, p.max);
  } else if (p.type === "range") {
    input = new UI.Range(p.min, p.max, value, p.step).setValue(
      parseFloat(value)
    );
    ...
    input = new UI.Select().setWidth("").setOptions(p.options).setValue(value);
  } else if (p.type === "color") {
    input = new UI.ColorPopupMenu(p.label).setValue(value);
  } else if (p.type === "vector3") {
    input = new UI.Vector3(value).setPrecision(p.precision);
  } else if (p.type === "hidden") {
    // nothing to display
  } else {
    console.warn(
      "NGL.createParameterInput: unknown parameter type " + "'" + p.type + "'"
    );
  }
...
NGL.StageWidget = function (stage, opts) {
  opts = opts || {};
  // ▶ 우리가 넘겨줄 래퍼(root). 없으면 기존과 동일하게 body.
  var root = opts.root || document.body;
  if (!root.id) root.id = "stage-root";
  root.style.position = root.style.position || "relative";
  // ▶ 리사이즈 계산에 쓸 경계(부모 기준). 기본은 root.getBoundingClientRect()
  var getBounds =
    opts.getBounds ||
    function () {
  ...
  // ---- OverlayPanel.attach 기본 부착 위치를 wrapper(root)로 강제 ----
  if (window.UI && window.UI.OverlayPanel && !window.UI.__patchedAttach) {
    window.UI.__patchedAttach = true; // 중복 패치 방지
    const origAttach = window.UI.OverlayPanel.prototype.attach;
    window.UI.OverlayPanel.prototype.attach = function (parent) {
      // parent가 명시되지 않으면 StageWidget의 root로
      const target = parent || root || document.body;
  ...
  // 기본값: 사이드바 숨김으로 시작
  var __initSidebarWidth = sidebar.dom.getBoundingClientRect().width || 300; // 나중에 다시 켤 때 쓸 기억값
  stage.__lastSidebarWidth = __initSidebarWidth; // 기억해둔다

  sidebar.dom.style.display = "none"; // 숨김
  viewport.dom.style.right = "0px"; // 오른쪽 여백 제거 → 전체 폭
  toolbar.dom.style.right = "0px";

  var tbH0 = toolbar.dom.getBoundingClientRect().height || 32; // 툴바 높이만큼 바닥 여백
  viewport.dom.style.bottom = tbH0 + "px";

  // 바로 한 번 리사이즈 반영
  stage.handleResize();

  stage.__getBounds = getBounds;
  stage.__rootPanel = root;
  ...
  var handleResizeLeft = function (clientX) {
    ...
    if (clientX >= 50 && clientX <= bounds.width - 10) {
      sidebar.setWidth(bounds.width - clientX + "px");
    ...
    // ▼ 추가: viewport/toolbar는 right 값으로 사이드바 폭을 ‘예약’해야 빈칸이 없어짐
    viewport.dom.style.right = sidebarWidth + "px";
    toolbar.dom.style.right = sidebarWidth + "px";
    stage.__lastSidebarWidth = sidebarWidth;
    var tbH =
      toolbar.dom.style.display === "none"
        ? 0
        : toolbar.dom.getBoundingClientRect().height || 32;
    viewport.dom.style.bottom = tbH + "px";

    if (clientX === undefined) {
      var mainWidth = bounds.width - sidebarWidth;
  ...
  handleResizeLeft = NGL.throttle(handleResizeLeft, 50, {
    leading: true,
    trailing: true,
  });
  stage.__handleResizeLeft = handleResizeLeft;
  ...
  var resizeLeft = new UI.Panel()
    ...
    .onClick(function () {
      ...
      if (minResizeLeft) {
        handleResizeLeft(w - 300);
      } else if (!doResizeLeft && !movedResizeLeft) {
        handleResizeLeft(w - 10);
      }
    });
  ...
  stage.__sidebarWidget = sidebar;
  stage.__viewportWidget = viewport;
  stage.__toolbarWidget = toolbar;
  stage.__lastSidebarWidth = sidebar.dom.getBoundingClientRect().width || 300;
...
NGL.ToolbarWidget = function (stage) {
  ...
  stage.viewer.stats.signals.updated.add(function () {
    if (NGL.Debug) {
      statsText.setValue(
        stage.viewer.stats.lastDuration.toFixed(2) +
          " ms | " +
          stage.viewer.stats.lastFps +
          " fps"
      );
    } else {
      statsText.setValue("");
...
NGL.MenubarWidget = function (stage, preferences) {
  ...
  container.add(new UI.Panel().setClass("menu").setFloat("right"));
...
NGL.MenubarFileWidget = function (stage) {
  ...
  function onImportOptionClick() {
    ...
    dirWidget.setOpacity("0.9").setLeft("50px").setTop("80px").attach();
  }
...
NGL.MenubarViewWidget = function (stage, preferences) {
  ...
  function onToggleSidebarClick() {
    var sw = stage.__sidebarWidget;
    var vp = stage.__viewportWidget;
    var tb = stage.__toolbarWidget;
    if (!sw || !vp || !tb) return;

    var hideSidebar = sw.dom.style.display !== "none";

    // 1) 사이드바 토글
    sw.dom.style.display = hideSidebar ? "none" : "";

    // 2) right 동기화 (이미 하고 있다면 유지)
    var w = hideSidebar
      ? 0
      : stage.__lastSidebarWidth || sw.dom.getBoundingClientRect().width || 300;
    vp.dom.style.right = w + "px";
    tb.dom.style.right = w + "px";

    // 3) 하단(툴바) 처리: 그대로 둘지/숨길지 선택

    // (옵션 i) 툴바 유지하되, viewport bottom은 툴바 높이만큼 비우기
    var tbH = tb.dom.getBoundingClientRect().height || 32;
    vp.dom.style.bottom = tbH + "px";

    // (옵션 ii) 사이드바 숨길 때 툴바까지 숨기고, 뷰포트는 바닥까지 채우기
    // if (hideSidebar) {
    //   tb.dom.style.display = 'none';
    //   vp.dom.style.bottom = '0px';
    // } else {
    //   tb.dom.style.display = '';
    //   var tbH = tb.dom.getBoundingClientRect().height || 32;
    //   vp.dom.style.bottom = tbH + 'px';
    // }

    // 리플로우 후 리사이즈
    void vp.dom.getBoundingClientRect();
    stage.handleResize();
  }
...
NGL.MenubarHelpWidget = function (stage, preferences) {
  ...
  var preferencesWidget = new NGL.PreferencesWidget(stage, preferences)
    .setDisplay("none")
    .attach(); // configure menu contents
...
NGL.ExportImageWidget = function (stage) {
  ...
  var antialiasCheckbox = new UI.Checkbox().setValue(true);

  var trimCheckbox = new UI.Checkbox().setValue(false);

  var transparentCheckbox = new UI.Checkbox().setValue(false);

  var progress = new UI.Progress().setDisplay("none");

  var exportButton = new UI.Button("export").onClick(function () {
...
NGL.SidebarWidget = function (stage) {
  ...
  var busy = new UI.Panel()
    .setDisplay("inline")
    .add(new UI.Icon("spinner").addClass("spin").setMarginLeft("45px"));
  ...
  var actions = new UI.Panel()
    ...
    .add(expandAll, collapseAll, centerAll, disposeAll, settingsMenu);
...
NGL.StructureComponentWidget = function (component, stage) {
    ...
    function onListingClick(info) {
      ...
      if (remoteTrajExt.indexOf(ext) !== -1) {
        component.addTrajectory(
          info.path + "?struc=" + component.structure.path
        );
        dirWidget.dispose();
      } else {
  ...
  var superpose = new UI.Select().setColor("#444").onChange(function () {
  ...
  container.addStatic(componentPanel).add(trajContainer).add(reprContainer);
...
NGL.SurfaceComponentWidget = function (component, stage) {
  ...
  var componentPanel = new UI.ComponentPanel(component)
    ...
    .addMenuEntry(
      "File",
      new UI.Text(component.surface.path)
        .setMaxWidth("100px")
        .setWordWrap("break-word")
    )
    .addMenuEntry("Position", position)
  ...
  container.addStatic(componentPanel).add(reprContainer);
...
NGL.VolumeComponentWidget = function (component, stage) {
  ...
  var componentPanel = new UI.ComponentPanel(component)
    ...
    .addMenuEntry(
      "File",
      new UI.Text(component.volume.path)
        .setMaxWidth("100px")
        .setWordWrap("break-word")
    )
    .addMenuEntry("Position", position)
...
NGL.ShapeComponentWidget = function (component, stage) {
  ...
  var componentPanel = new UI.ComponentPanel(component)
    ...
    .addMenuEntry(
      "File",
      new UI.Text(component.shape.path)
        .setMaxWidth("100px")
        .setWordWrap("break-word")
    )
    .addMenuEntry("Position", position)
...
NGL.RepresentationElementWidget = function (element, stage) {
  ...
  var container = new UI.CollapsibleIconPanel(
    "minus-square",
    "plus-square"
  ).setMarginLeft("20px");
  ...
  var name = new UI.EllipsisText(element.name).setWidth("103px");
  ...
  container.addStatic(name).addStatic(toggle).addStatic(disposeIcon);

  // Selection

  if (
    (element.parent.type === "structure" ||
      element.parent.type === "trajectory") &&
    element.repr.selection &&
    element.repr.selection.type === "selection"
  ) {
    container.add(
...
NGL.TrajectoryElementWidget = function (element, stage) {
  ...
  var frameCount = new UI.Panel()
    .setDisplay("inline")
    .add(new UI.Icon("spinner").addClass("spin").setMarginRight("99px"));

  var frameTime = new UI.Panel().setMarginLeft("5px").setDisplay("inline");
  ...
  var name = new UI.EllipsisText(element.parameters.name).setWidth("103px");
  ...
  var time = new UI.Text().setTextAlign("right").setWidth("90px");
  ...
  var interpolateStep = new UI.Integer(
    element.parameters.defaultInterpolateStep
  )
    .setWidth("30px")
    .setRange(1, 50)
    .onChange(function () {
  ...
  player.signals.startedRunning.add(function () {
    playerButton.setTitle("pause").setValue(false);
  });

  player.signals.haltedRunning.add(function () {
    playerButton.setTitle("play").setValue(true);
  });
  ...
  var setCenterPbc = new UI.Checkbox(traj.centerPbc).onChange(function () {
    element.setParameters({
      centerPbc: setCenterPbc.getValue(),
    });
  });

  var setRemovePeriodicity = new UI.Checkbox(traj.removePeriodicity).onChange(
    function () {
      element.setParameters({
  ...
  var setRemovePbc = new UI.Checkbox(traj.removePbc).onChange(function () {
    element.setParameters({
      removePbc: setRemovePbc.getValue(),
    });
  });

  var setSuperpose = new UI.Checkbox(traj.superpose).onChange(function () {
    element.setParameters({
  ...
  var dispose = new UI.DisposeIcon().setDisposeFunction(function () {
  ...
  var menu = new UI.PopupMenu("bars", "Trajectory")
    ...
    .addEntry(
      "File",
      new UI.Text(traj.trajPath).setMaxWidth("100px").setWordWrap("break-word")
    )
...
NGL.DirectoryListingWidget = function (
  datasource,
  stage,
  heading,
  filter,
  ...
  var folderSelect = new UI.Select()
    ...
    .onChange(function () {
      datasource.getListing(folderSelect.getValue()).then(onListingLoaded);
    });
  ...
  function onListingLoaded(listing) {
    ...
    folderSelect.setOptions(getFolderDict(folder)).setValue(folder);
    ...
    data.forEach(function (info) {
      ...
      var pathRow = new UI.Panel()
        ...
        .onClick(function () {
          if (info.dir) {
            datasource.getListing(info.path).then(onListingLoaded);
          } else {
            callback(info);
          }
        });
  ...
  datasource.getListing(NGL.lastUsedDirectory).then(onListingLoaded);
